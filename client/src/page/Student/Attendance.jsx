...
function Attendance() {
  ...
  // Close calendar when clicking outside (handled by DayPicker's own logic via open state)
  ...
  const getPeriodPageNumbers = () => {
    ...
    if (totalPeriodPages <= 5) {
      for (let i = 1; i <= totalPeriodPages; i++) pages.push(i);
    } else if (periodCurrentPage <= 3) {
      for (let i = 1; i <= 5; i++) pages.push(i);
    } else if (periodCurrentPage >= totalPeriodPages - 2) {
      for (let i = totalPeriodPages - 4; i <= totalPeriodPages; i++) pages.push(i);
    } else {
      for (let i = periodCurrentPage - 2; i <= periodCurrentPage + 2; i++) pages.push(i);
    }
    return pages;
  };
  ...
  function ExamPerformanceTrendChart() {
    ...
    useEffect(() => {
      ...
      grid.selectAll("line").attr("stroke", "#e6e6e6").attr("stroke-dasharray", "2 2");
      grid.selectAll("line").filter((d) => d === 100).remove();
      ...
      chart.append("path").datum(data).attr("fill", "url(#area-gradient)").attr("d", area);
      ...
      chart
        ...
        .on("mouseenter", (event, d) => {
          ...
          d3.select(tooltipRef.current)
            .style("opacity", 1)
            .html(`<strong>${d.Attendance}</strong><br/>attendanceRate: ${d.attendanceRate}`);
        })
        .on("mousemove", (event) => {
  ...
  return (
    <div className="">
      {/* TOP CARDS — only on Daily Attendance */}
      {active === "Daily Attendance" && <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-3 gap-y-3">
        {attendanceData.map((item, index) => (
          <div key={index} className="box-shadow bg-white rounded-2xl p-4">
            <div className="flex items-center justify-between ">
              <div className="flex flex-col justify-center">
                <p className="font-medium text-[16px] text-[#1C1C1C]">{item.title}</p>
                ...
                <p className="mt-3 flex items-center gap-2 text-[14px] font-semibold ">
                  <span className={`${item.percentColor}`}>{item.percentage}</span>
                  <span className="text-[#696969]">{item.des}</span>
                </p>
              </div>
              <div className="flex items-center justify-center w-16 h-16">
                <img src={item.icon} alt="student" className="w-full h-full object-contain" />
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="shadow-md mt-3 bg-white rounded-md">
            ...
            <div className="p-4 flex items-center gap-36">
              ...
              <div className="inline-flex items-center gap-3">
                ...
                <div className="bg-[#EFF2F2] rounded px-1 py-1">
                  <select name="" id="Section" className="px-1 py-1 border-0 outline-0">
                ...
                <table className="w-full">
                  <thead className="bg-[#F5F7F7]">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-sm font-semibold">Attandance%</th>
                    ...
                    {studentData.map((item, index) => (
                      <tr key={index} className="border-b border-[#e6e6e6]">
                        <td className="px-4 py-3 text-left text-sm font-semibold text-[#9c9c9c]">
                          <div className="flex gap-3 items-center">#{item.rollNo}</div>
                        </td>
                        <td className="px-4 py-3 text-left text-sm font-semibold">
                          <div className="flex gap-4 items-center">
                            ...
                            <div className="flex flex-col leading-tight">
                              <span className="text-[#12516E] font-semibold">{item.student}</span>
        ...
        {/* ===================== PERIOD ATTENDANCE ===================== */}
        {active === "Period Attendance" && (
          <div className="p-4">
            {/* CLASS & PERIOD FILTERS */}
            <div className="flex flex-col md:flex-row gap-4 mt-4">
              {/* Class Dropdown */}
              <div className="flex-1">
                <label className="block text-[14px] font-semibold font-[600] text-[#1C1C1C] mb-1">
                  Class
                </label>
              ...
              {/* Period Dropdown */}
              <div className="flex-1">
                <label className="block text-[14px] font-[600] font-semibold text-[#1C1C1C] mb-1">
                  Period
                </label>
            ...
            {/* PERIOD ATTENDANCE TABLE — shown after Search */}
                    ...
                    <table className="w-full table-auto min-w-[900px]">
                      ...
                      <tbody>
                        {(() => {
                          ...
                          const currentPeriodData = filteredPeriodData.slice(
                            indexOfFirstPeriod,
                            indexOfLastPeriod
                          );
                          ...
                          if (currentPeriodData.length > 0) {
                            return currentPeriodData.map((item) => (
                              <tr
                                key={item.id}
                                className="border-b border-[#e6e6e6] transition-colors"
                              >
                                ...
                                <td className="pl-20 pr-5 py-3 text-center text-[14px] font-semibold whitespace-nowrap">
                                  ...
                                  >
                                    ...
                                    <span className="font-semibold font-[600] leading-none">
                                      {item.status}
                                    </span>
                                  </span>
                                </td>
                                ...
                                <td className="pl-1 py-3 text-center">
                                  <div className="flex items-center justify-center gap-3">
                                    <button
                                      onClick={() =>
                                        handlePeriodStatusChange(item.id, "Present")
                                      }
                                    ...
                                    <button
                                      onClick={() =>
                                        handlePeriodStatusChange(item.id, "Absent")
                                      }
                                      className={`border w-[150px] h-[24px] rounded-[4px] font-normal text-[14px] transition-colors ${
                                        item.status === "Absent"
                            ...
                            return (
                              <tr>
                                <td
                                  colSpan="5"
                                  className="text-center py-8 text-[#9C9C9C]"
                                >
                                  No students found
                                </td>
                              </tr>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="relative">
                            <div
                              onClick={() =>
                                setIsPeriodItemsPerPageOpen(!isPeriodItemsPerPageOpen)
                              }
                              style={{
                                width: "80px",
                                padding: "4px 8px",
                            ...
                            >
                              <span className="text-[#1C1C1C] font-bold text-[14px]">
                                {periodItemsPerPage}
                              </span>
                              <span
                                style={{
                                  ...
                                  transform: isPeriodItemsPerPageOpen
                                    ? "rotate(180deg)"
                                    : "rotate(0deg)",
                                }}
                              >
                          ...
                          <span className="text-[14px] text-[#696969]">
                            ...
                            <span style={{ color: "#1C1C1C" }}>
                              {filteredPeriodData.length === 0
                                ? 0
                                : (periodCurrentPage - 1) * periodItemsPerPage + 1}
                            </span>
                            -{Math.min(
                              periodCurrentPage * periodItemsPerPage,
                              filteredPeriodData.length
                            )}{" "}
                            of {filteredPeriodData.length} results
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                          <div
                            ...
                            className={`cursor-pointer ${
                              periodCurrentPage === 1 ? "opacity-30 pointer-events-none" : ""
                            }`}
                          >
                            <button
                          ...
                          <div
                            ...
                            className={`cursor-pointer ${
                              periodCurrentPage ===
                                Math.ceil(filteredPeriodData.length / periodItemsPerPage) ||
                              filteredPeriodData.length === 0
      ...
      {/* BOTTOM 2 CARDS — only on Daily Attendance */}
      {active === "Daily Attendance" && <div className="flex flex-col lg:flex-row gap-4 mt-6">
        {/* 45% */}
                ...
                >
                  ...
                  <div className="flex justify-between w-full">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[#9c9c9c] text-[14px] font-normal">{item.class}</span>
        ...
        <div className="w-full lg:w-[55%] bg-white p-4 rounded-lg shadow">
          <div className="flex justify-between items-center">
            <span className="font-semibold text-[#0B3142]">Attandance trend</span>
